    ...
    >
      ...
      >
        ...
        <form onSubmit={submit} className="p-6 space-y-4">
          <div>
            <label className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-2 block">Target Audience</label>
            <select 
              value={target.tier} 
            ...
            >
              <option value="" className="bg-[#0c0e14]">All Active Customers</option>
              <option value="0" className="bg-[#0c0e14]">Member Only</option>
              <option value="1" className="bg-[#0c0e14]">Silver Only</option>
              <option value="2" className="bg-[#0c0e14]">Gold Only</option>
              <option value="3" className="bg-[#0c0e14]">Diamond VIP Only</option>
            </select>
          </div>

          <div>
            <label className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-2 block">Message Content</label>
            <textarea 
              value={content}
              onChange={e => setContent(e.target.value)}
              placeholder="Hi {{name}}! We have a special gift for you today..."
    ...
    <div className="space-y-8 pb-10">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6">
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
          ...
          <h1 className="text-4xl font-black mt-2 tracking-tight">Campaigns & <span className="bg-clip-text text-transparent bg-gradient-to-r from-yellow-400 to-amber-500">Promotions</span></h1>
        </motion.div>
        ...
        <div className="flex flex-wrap gap-4">
          <div className="relative">
            ...
            <input 
              type="text" placeholder="Search codes, desc..."
          ...
          >
            <MessageSquareShare size={18} /> Bulk SMS
          </button>
          <motion.button 
          ...
          >
            <div className="bg-background/90 rounded-[15px] px-6 py-3 flex items-center gap-3">
              ...
              <span className="text-sm font-bold text-yellow-400">Launch Campaign</span>
            </div>
          </motion.button>
        </div>
      </div>
      ...
      >
        {[
          { label: 'Total Vouchers', value: stats.total, icon: <Ticket size={20} />, color: 'text-blue-400', bg: 'bg-blue-500/10 border-blue-500/20' },
          { label: 'Active', value: stats.active, icon: <Clock size={20} />, color: 'text-yellow-400', bg: 'bg-yellow-500/10 border-yellow-500/20' },
          { label: 'Redeemed', value: stats.used, icon: <CheckCircle2 size={20} />, color: 'text-emerald-400', bg: 'bg-emerald-500/10 border-emerald-500/20' },
          { label: 'Conversion', value: stats.total > 0 ? `${((stats.used / stats.total) * 100).toFixed(1)}%` : '0%', icon: <TrendingUp size={20} />, color: 'text-violet-400', bg: 'bg-violet-500/10 border-violet-500/20' },
        ].map((stat) => (
        ...
        >
          {isLoading ? (
            <div className="p-20 flex flex-col items-center justify-center">
              ...
              <p className="text-muted-foreground animate-pulse font-bold tracking-widest uppercase text-[10px]">Retrieving Global Vouchers...</p>
            </div>
          ) : displayedVouchers.length === 0 ? (
            <div className="p-20 flex flex-col items-center justify-center text-center">
              <Tag size={60} className="text-white/10 mb-4" />
              <h3 className="text-xl font-bold mb-2">No active campaigns</h3>
              <p className="text-muted-foreground text-sm">Start your first loyalty program using the Launch button.</p>
            </div>
          ) : (
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="border-b border-white/5 bg-white/[0.03]">
                    <th className="py-4 px-6 text-[10px] font-black text-muted-foreground tracking-[0.2em] uppercase">Voucher Code</th>
                    <th className="py-4 px-6 text-[10px] font-black text-muted-foreground tracking-[0.2em] uppercase">Description</th>
                    <th className="py-4 px-6 text-[10px] font-black text-muted-foreground tracking-[0.2em] uppercase">Value</th>
                    <th className="py-4 px-6 text-[10px] font-black text-muted-foreground tracking-[0.2em] uppercase">Receiver</th>
                    <th className="py-4 px-6 text-[10px] font-black text-muted-foreground tracking-[0.2em] uppercase">Status</th>
                  </tr>
                </thead>
                <tbody>
                  <AnimatePresence>
                    {displayedVouchers.map((v, idx) => {
                       ...
                       return (
                        ...
                        >
                          ...
                          <td className="py-4 px-6 text-sm font-medium">{v.description || 'Campaign gift'}</td>
                          <td className="py-4 px-6">
                            <span className="font-black text-emerald-400">
                                {v.discountType === 0 ? `${v.value}%` : `$${v.value.toLocaleString()}`}
                            </span>
                          </td>
                          <td className="py-4 px-6">
                             <div className="text-xs font-bold">{v.customer?.fullName || 'Bulk Gift'}</div>
                             <div className="text-[10px] text-muted-foreground">{v.customer?.phoneNumber || 'Multiple Customers'}</div>
                          </td>
                          <td className="py-4 px-6">
                            {v.isUsed ? (
                              ...
                              <span className="text-[10px] font-black uppercase text-red-400 bg-red-400/10 px-2 py-1 rounded-full border border-red-400/30 w-fit">Expired</span>
                            ) : (
                              <span className="text-[10px] font-black uppercase text-yellow-400 bg-yellow-400/10 px-2 py-1 rounded-full border border-yellow-400/30 w-fit">Active</span>
                            )}
                          </td>
                        </motion.tr>
                      );
        ...
        <div className="lg:w-1/3 space-y-6">
           ...
           >
              <h3 className="text-lg font-bold mb-4 flex items-center gap-2">
                 <Sparkles size={18} className="text-yellow-400" /> Marketing Secrets
              </h3>
              <div className="space-y-4">
                 {[
                   { title: 'Best Time to Blast', desc: 'Saturdays at 11:00 AM usually yields 25% higher open rates.', icon: <Calendar size={16} /> },
                   { title: 'Personalize Content', desc: 'Use {{name}} tag to increase voucher redemption by 30%.', icon: <Gift size={16} /> },
                   { title: 'Urgency Matters', desc: 'Vouchers expiring in < 3 days have the fastest turnaround.', icon: <Clock size={16} /> },
                 ].map((t, i) => (
           ...
           >
              <h3 className="text-lg font-bold mb-4">Pro Loyalty Tip</h3>
              <p className="text-xs text-muted-foreground leading-relaxed">
                 Segments are key! Only send **DIAMOND** vouchers to high-spenders to maintain brand exclusivity. Use the **Bulk SMS** tool to notify them quietly.
              </p>
           </motion.div>
        </div>
      </div>
